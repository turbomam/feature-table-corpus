...
`just lakehouse-export` writes Parquet under `local/` with the toolchain pinned in
`requirements-lakehouse.txt`; see the [Parquet export guide](lakehouse-export.md).
