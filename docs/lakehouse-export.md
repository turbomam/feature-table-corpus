...
tracked files. Nothing appears at `DIR` unless both checks below pass.
...
without a code change. Each file has one column per slot of its class, in schema order. A
collection with no rows still gets a file with every column, so the files for different
Datasets have the same columns.

## Types

linkml-store creates each table from the schema, but three of its default mappings lose
information. The export widens those before loading and casts inlined objects to nested
Parquet types. Everything else is linkml-store's own layout.
...
| `boolean` (`is_selected`) | `VARCHAR` holding `"true"` | `bool` |
...
as a negative control. The 4-byte integer is not wrong for any current input (it holds
positions up to 2,147,483,647); it is widened to match `build_duckdb.py`'s `BIGINT`.
...
Both run before `DIR` is published, and a failure leaves no output:
...
value, drop one row, and export with linkml-store's default types; each must be reported.

## Measurements

One run each on 2026-09-25. "Checks" is mostly `build_duckdb.py` validating the Dataset
again and inserting row by row.

| Dataset | Features | Dataset JSON | `features.parquet` | Validate | Load and write | Checks | Wall clock |
|---|---|---|---|---|---|---|---|
| One-biosample example | 15 | YAML | 9,737 B | 0.07 s | 0.04 s | 0.58 s | not timed |
| Constructed IMG fixture | 10 | 7,908 B | 5,895 B | 0.06 s | 0.03 s | 0.05 s | 0.84 s |
| `Ga0416744` (IMG_AP-1257706) | 2,005 | 2,426,519 B | 274,639 B | 2.05 s | 0.16 s | 2.43 s | 5.44 s |
| `Ga0423362` (IMG_AP-1268149) | 4,439 | 5,371,707 B | 595,593 B | 4.83 s | 0.35 s | 5.83 s | 11.74 s |
