...
class SchemaLintTests(unittest.TestCase):
    def test_real_schema_has_only_allowed_findings(self):
        self.assertEqual(lint(ROOT / "schema/ber_feature_model.yaml"), [])

    def test_replacing_allowed_warning_still_fails(self):
        schema = yaml.safe_load((ROOT / "schema/ber_feature_model.yaml").read_text())
        ...
        with tempfile.TemporaryDirectory(dir=scratch) as directory:
            ...
            (Path(directory) / "attributes.yaml").write_bytes((ROOT / "schema/attributes.yaml").read_bytes())
